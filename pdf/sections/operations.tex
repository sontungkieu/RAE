...
\begin{itemize}[leftmargin=1.5em]
  ...
  \texttt{vaes-jax-celeba-kaggle-tpuv5e8-sitb-moe1.ipynb}. Bản này chuẩn bị trực tiếp
  \texttt{celeba256\_imgfolder} từ dataset Kaggle, rồi đổi Stage 1 sang
  \texttt{stage1.StabilityVAE}, đổi Stage 2 sang single-tower
  \texttt{SiT-B}, thêm learned source \texttt{moe1}, bỏ hẳn bước bootstrap
  identity stat, và chèn pass \texttt{build\_source\_gmm.py} trước cell train.
  \item Nếu phần cứng là Kaggle \texttt{TPU v5e-8} và cần flow VAE đó trên TPU,
  dùng \texttt{vaes-jax-celeba-kaggle-tpuv5e8-sitb-moe1.ipynb}. Notebook này giữ
  toàn bộ fix Kaggle/JAX hiện có, bật \texttt{training.random\_flip=true},
  viết config Stage 2 thành
  \texttt{CelebA256\_SiT-B\_StabilityVAE\_moe1\_jax\_tpuv5e8.yaml}, và dùng
  shape single-tower \texttt{hidden\_size=768}, \texttt{depth=12},
  \texttt{num\_heads=12}. Các cell train mặc định bật
  ...
  \texttt{training.prefetch\_factor=4}, và \texttt{eval.prefetch\_factor=4}.
  \item Nếu đã có run Orbax của flow \texttt{StabilityVAE + SiT-B + moe1} và cần
  resume, dùng notebook
  \texttt{vaes-jax-celeba-kaggle-tpuv5e8-sitb-moe1-resume.ipynb}. Notebook này
  sẽ copy \texttt{checkpoint\_*} mới nhất từ
  \texttt{CelebA256\_SiT-B\_StabilityVAE\_moe1\_jax\_tpuv5e8-*} sang workdir
  resume mới có timestamp, tạo \texttt{wandb\_run.json} với run ID W\&B mới
  nhưng vẫn giữ \texttt{exp\_name} gốc, rồi mới ép lại
  \texttt{training.log\_rae\_latent\_stats=true} cùng
