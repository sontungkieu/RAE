...
\begin{itemize}[leftmargin=1.5em]
  ...
  \item Nếu chạy trên Kaggle với CelebA, bắt đầu từ notebook TPU
  \texttt{raes-jax-celeba-kaggle-tpuv5e8-sitdh-b.ipynb} để giữ đúng flow clone repo, đặt
  ...
  \texttt{raes-jax-celeba-kaggle-tpuv5e8-sitdh-b.ipynb}; bản này cố định biến
  thể Stage 2 là \texttt{SiTDH-B}, chuyển FID và build reference stats sang
  ...
  \texttt{/kaggle/working/results\_jax\_tpu/}, copy
  \texttt{checkpoint\_<step>/} mới nhất sang workdir resume mới có timestamp,
  tạo \texttt{wandb\_run.json} với run ID W\&B mới, rồi mới truyền
  \texttt{--workdir} vào \texttt{src\_jax/train.py}. Runtime cũng tiếp tục
  restore bằng \texttt{latest\_step()} từ checkpoint đã copy, còn notebook vẫn
  giữ \texttt{exp\_name} gốc nên không còn hardcode
  \texttt{checkpoint\_100000}.
  \item Branch này cũng giữ song song bộ notebook TPU CelebA-HQ:
  \texttt{raes-jax-celebahq-kaggle-tpuv5e8-sitdh-b.ipynb}, và
  \texttt{raes-jax-celebahq-kaggle-tpuv5e8-sitdh-b-resume.ipynb}. Merge này
  cũng khôi phục hai helper export riêng cho CelebA-HQ là
