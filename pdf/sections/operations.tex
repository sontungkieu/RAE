...
\begin{itemize}[leftmargin=1.5em]
  ...
  \texttt{vaes-jax-celeba-kaggle-tpuv5e8-sitb.ipynb}. Bản này dựng
  \texttt{celeba256\_imgfolder} từ dataset Kaggle giống branch
  \texttt{jax-sit-dh}, nhưng đổi Stage 1 sang \texttt{stage1.StabilityVAE},
  đổi Stage 2 sang single-tower \texttt{SiT-B}, và bỏ hẳn bước bootstrap
  identity stat cùng pass \texttt{build\_stage1\_stats.py} bắt buộc.
  \item Nếu phần cứng là Kaggle \texttt{TPU v5e-8} và cần flow VAE đó trên TPU,
  dùng \texttt{vaes-jax-celeba-kaggle-tpuv5e8-sitb.ipynb}. Notebook này giữ
  toàn bộ fix Kaggle/JAX hiện có, bật \texttt{training.random\_flip=true},
  viết config Stage 2 thành
  \texttt{CelebA256\_SiT-B\_StabilityVAE\_jax\_tpuv5e8.yaml}, bật luôn
  \texttt{training.log\_rae\_latent\_stats=true} và
  \texttt{training.log\_activation\_stats=true}, mặc định
  \texttt{num\_workers=16} và \texttt{prefetch\_factor=4} cho cả train/eval,
  rồi dùng shape single-tower \texttt{hidden\_size=768}, \texttt{depth=12},
  \texttt{num\_heads=12}.
  \item Nếu đã có run Orbax của flow \texttt{StabilityVAE + SiT-B} và cần
  resume, dùng notebook
  \texttt{vaes-jax-celeba-kaggle-tpuv5e8-sitb-resume.ipynb}. Notebook này
  sẽ copy \texttt{checkpoint\_*} mới nhất từ
  \texttt{CelebA256\_SiT-B\_StabilityVAE\_jax\_tpuv5e8-*} sang workdir resume
  mới có timestamp, tạo \texttt{wandb\_run.json} với run ID W\&B mới nhưng vẫn
  giữ \texttt{exp\_name} gốc, rồi mới ép lại cả cấu hình
  \texttt{num\_workers=16}, \texttt{prefetch\_factor=4}, và hai cờ log
  diagnostics khi resume.
