...
\begin{itemize}[leftmargin=1.5em]
  ...
  \texttt{var\_kl\_loss\_weight}: trọng số auxiliary losses phía source. Các
  default hiện tại được kéo gần public recipe của
  \texttt{shortcut-models@moe1}: router mềm hơn với
  \texttt{router\_temperature=2.0}, độ rộng \texttt{16/64}, và regularization
  variance mạnh hơn với \texttt{var\_kl\_loss\_weight=1.0};
...
\begin{itemize}[leftmargin=1.5em]
  ...
  ảnh đi qua Stage 1; các config CelebA public của branch này mặc định bật cờ
  này cho train nếu không override;
  ...
  \item public notebook CelebA \texttt{moe1} override các default generic này
  sang \texttt{training.num\_workers=16}, \texttt{eval.num\_workers=16},
  \texttt{training.prefetch\_factor=4}, \texttt{eval.prefetch\_factor=4},
