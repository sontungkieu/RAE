...
Stage 1 sang \texttt{stage1.StabilityVAE} và Stage 2 sang single-tower
\texttt{stage2.models.SiT.SiT}. Các config được sinh runtime là:

\begin{itemize}[leftmargin=1.5em]
  \item \texttt{configs/stage1/pretrained/CelebAHQ256\_StabilityVAE\_jax.yaml};
  \item \texttt{configs/stage2/training/CelebAHQ256\_SiT-B\_StabilityVAE\_jax.yaml}.
\end{itemize}
...
2 thành \texttt{CelebAHQ256\_SiT-B\_StabilityVAE\_jax\_tpuv5e8.yaml}, dùng
...
\texttt{CelebAHQ256\_SiT-B\_StabilityVAE\_jax\_tpuv5e8-*}. Nếu workdir đích là
legacy run được tạo trước khi cơ chế \texttt{wandb\_run.json} tồn tại, hãy
export \texttt{WANDB\_RUN\_ID=<existing\_run\_id>} một lần để notebook resume
tự thêm \texttt{--wandb-run-id} khi gọi lệnh train.
