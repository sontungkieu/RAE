...
Repo hiện có notebook \texttt{raes-jax-celeba-kaggle-moe1.ipynb} để bám đúng
flow Kaggle của branch learned-source:
...
\texttt{/kaggle/working/celeba256\_imgfolder}. Notebook GPU train trực tiếp từ
split \texttt{train}, còn hai notebook TPU dùng root \texttt{ImageFolder} và
vẫn giữ \texttt{eval.data\_path} trên split \texttt{val}. Các notebook TPU
...
\texttt{src\_jax/build\_source\_gmm.py} trên branch này cũng mặc định dùng
\texttt{feature\_extractor=auto}; với latent RAE \texttt{16 x 16 x 768}, nó sẽ
chọn \texttt{pyramid\_16k}, tức pool/proj latent về \texttt{4 x 4 x 1024} rồi
flatten thành \texttt{16384} chiều trước khi fit GMM. Trên máy RAM lớn có thể
ép \texttt{flatten}; builder giờ preallocate đúng một ma trận
\texttt{N x D} \texttt{float32} và standardize in-place, nên full latent vector
không còn cần memmap trên đĩa chỉ để né thêm bản copy RAM. Nếu máy không đủ
RAM thì mới ép xuống \texttt{spatial\_mean} làm fallback tiết kiệm bộ nhớ.
...
\texttt{SiTDH-B + moe1},
dùng notebook \texttt{raes-jax-celeba-kaggle-tpuv5e8-sitdh-b-moe1.ipynb}. Notebook
này ghi config CelebA thành
\texttt{CelebA256\_SiTDH-B\_DINOv2-B\_moe1\_jax\_tpuv5e8.yaml}, đặt backbone DH thành
...
thể \texttt{SiTDH-B}, build artifact GMM trước train, và giữ cùng cadence
checkpoint \texttt{210000} step. Block \texttt{source} của notebook này cũng
dùng \texttt{condition\_dim=16}, \texttt{hidden\_channels=256},
\texttt{router\_temperature=2.0}, \texttt{balance\_loss\_weight=0.1},
\texttt{entropy\_loss\_weight=1.0e-2}, và
\texttt{var\_kl\_loss\_weight=1.0}; cell build FID stats vẫn giữ
\texttt{--num-workers 32}.
...
Branch này cũng giữ song song bộ notebook CelebA-HQ cho learned-source:
\texttt{raes-jax-celebahq-kaggle-moe1.ipynb},
\texttt{raes-jax-celebahq-kaggle-tpuv5e8-sitdh-s-moe1.ipynb},
