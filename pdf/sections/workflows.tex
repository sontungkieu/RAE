...
Nhánh này giữ một public notebook flow cho CelebA-HQ:
...
\texttt{vaes-jax-celebahq-kaggle-moe1.ipynb}. Notebook này giữ nguyên flow
\texttt{uv sync} và export Hugging Face sang \texttt{ImageFolder}, nhưng đổi
...
\begin{itemize}[leftmargin=1.5em]
  \item \texttt{configs/stage1/pretrained/CelebAHQ256\_StabilityVAE\_jax.yaml};
  \item \texttt{configs/stage2/training/CelebAHQ256\_SiT-B\_StabilityVAE\_moe1\_jax.yaml};
  \item \texttt{/kaggle/working/celebahq256\_source\_gmm.npz}.
\end{itemize}
...
\begin{itemize}[leftmargin=1.5em]
  ...
  \item cell train cuối truyền \texttt{--data-path /kaggle/working/celebahq256\_imgfolder},
  còn \texttt{eval.data\_path} trong config sinh ra vẫn trỏ tới split
  \texttt{val}.
\end{itemize}

Nếu chạy trên Kaggle \texttt{TPU v5e-8}, dùng notebook
\texttt{vaes-jax-celebahq-kaggle-tpuv5e8-sitb-moe1.ipynb}. Bản này ghi config Stage
2 thành \texttt{CelebAHQ256\_SiT-B\_StabilityVAE\_moe1\_jax\_tpuv5e8.yaml}, dùng
...
\texttt{training.prefetch\_factor=8} và \texttt{eval.prefetch\_factor=4} để
...
diagnostics của VAE, RMS/variance cho activation của SiT, source metrics theo
mặc định, cùng \texttt{source\_logvar\_mean/source\_var\_mean} để theo dõi độ
ổn định variance của learned source.
Cell train/resume của chúng cũng export \texttt{RAE\_JAX\_REBUILD\_BACKEND=1}
để session Kaggle luôn refresh cached backend overlay trước khi train với
\texttt{sit\_gmm\_moe1}.

Nếu đã có run Orbax của flow VAE và cần train tiếp, dùng notebook
\texttt{vaes-jax-celebahq-kaggle-tpuv5e8-sitb-moe1-resume.ipynb}. Notebook này bám
theo pattern resume-only của branch, nhưng tìm workdir mới nhất theo pattern
\texttt{CelebAHQ256\_SiT-B\_StabilityVAE\_moe1\_jax\_tpuv5e8-*}. Cell train
...
diagnostics mặc định, đồng thời export \texttt{RAE\_JAX\_REBUILD\_BACKEND=1}
trước khi gọi \texttt{src\_jax/train.py}. Nếu workdir đích là legacy run được
tạo trước khi cơ chế \texttt{wandb\_run.json} tồn tại, cần thêm
