...
Các notebook Stage 2 DH cho cả CelebA và CelebA-HQ được ship sẵn trên branch
này cũng đã pin cùng bộ override loader \texttt{16 / 4 / 4} và giữ hai nhóm
log chẩn đoán latent/activation bật mặc định.
...
Adapter giờ cũng coi \texttt{train\_step} là trục W\&B chuẩn cho train, eval,
FID, và media log, rồi mirror chính giá trị này sang internal \texttt{Step}
...
Repo hiện ship notebook TPU \texttt{raes-jax-celeba-kaggle-tpuv5e8-sitdh-b.ipynb}
để bám đúng flow Kaggle:
...
\texttt{raes-jax-celeba-kaggle-tpuv5e8-sitdh-b.ipynb}. Bản này cố định biến thể
CelebA Stage 2 là \texttt{SiTDH-B}, sync dependency của repo vào
...
Notebook này ghi config CelebA thành
...
\texttt{/kaggle/working/results\_jax\_tpu/}, copy thư mục
\texttt{checkpoint\_<step>/} mới nhất từ source workdir đó sang một workdir
resume mới có timestamp, ghi sẵn \texttt{wandb\_run.json} với run ID W\&B mới,
rồi mới truyền \texttt{--workdir} vào \texttt{src\_jax/train.py}. Runtime tiếp
tục restore \texttt{latest\_step()} từ checkpoint đã copy, còn notebook vẫn giữ
\texttt{exp\_name} gốc để dễ truy vết. Vì vậy notebook này không còn hardcode
cả thư mục run có timestamp lẫn \texttt{checkpoint\_100000}.

Branch này cũng giữ song song workflow TPU CelebA-HQ:
...
pipeline JAX/Kaggle tương tự nhưng export dữ liệu CelebA-HQ 256. Notebook
resume CelebA-HQ cũng đi theo flow copy checkpoint sang workdir mới, tạo run ID
W\&B mới, nhưng vẫn giữ nguyên tên thí nghiệm gốc. Merge này cũng khôi phục hai
helper export riêng là
