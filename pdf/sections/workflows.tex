...
Repo hiện có notebook \texttt{raes-jax-celebahq-kaggle-moe1.ipynb} để bám đúng
flow Kaggle của branch learned-source:

\begin{itemize}[leftmargin=1.5em]
  \item clone repo và checkout branch \texttt{jax-sit-dh-moe1-celebahq256};
  ...
  \item ghi sẵn config CelebA-HQ với block \texttt{source.enabled=true} cho \texttt{src\_jax/train.py}.
\end{itemize}
...
\texttt{eval.prefetch\_factor=4}. Các notebook \texttt{moe1} còn bật
\texttt{training.log\_rae\_latent\_stats=true},
...
\texttt{raes-jax-celebahq-kaggle-tpuv5e8-sitdh-s-moe1.ipynb}. Bản này cố định biến thể
CelebA-HQ Stage 2 là \texttt{SiTDH-S + moe1}, sync dependency của repo vào
...
\texttt{celebahq256\_source\_gmm.npz}, và giữ cadence checkpoint mặc định ở mốc
\texttt{210000} step. Config train CelebA-HQ của notebook này cũng bật
\texttt{training.random\_flip=true}. Block \texttt{source} sinh ra dùng
...
\texttt{SiTDH-B + moe1},
dùng notebook \texttt{raes-jax-celebahq-kaggle-tpuv5e8-sitdh-b-moe1.ipynb}. Notebook
này ghi config CelebA-HQ thành
\texttt{CelebAHQ256\_SiTDH-B\_DINOv2-B\_moe1\_jax\_tpuv5e8.yaml}, đặt backbone DH thành
...
\texttt{SiTDH-B}, build artifact GMM trước train, và giữ cùng cadence
...
notebook \texttt{raes-jax-celebahq-kaggle-tpuv5e8-sitdh-b-moe1-resume.ipynb}. Notebook
