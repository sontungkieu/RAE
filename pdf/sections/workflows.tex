...
\texttt{src\_jax/build\_source\_gmm.py} trên branch này giờ mặc định dùng
\texttt{feature\_extractor=flatten}; với latent RAE \texttt{16 x 16 x 768}, nó
giữ luôn vector đầy đủ \texttt{196608} chiều khi fit GMM. Builder vẫn đi theo
đường low-copy, nhưng \texttt{--storage-dtype auto} sẽ chọn
\texttt{float16} cho backing matrix khi flatten DH quá lớn, còn
\texttt{--compute-dtype auto} sẽ upcast từng chunk EM/KMeans về
\texttt{float32}. Nếu ai đó cố ép \texttt{--compute-dtype float16} cho full
flatten DH quá rộng, fitter giờ cũng sẽ tự promote phần KMeans/EM offline về
\texttt{float32} để tránh overflow và NaN treo lâu. Nhờ vậy full latent vector
không còn bắt buộc phải ở \texttt{float32} toàn cục chỉ để fit GMM. Nếu muốn
bản nén có cấu trúc hơn thì vẫn có thể ép \texttt{pyramid\_16k};
\texttt{spatial\_mean} tiếp tục là fallback tiết kiệm bộ nhớ khi máy không đủ
RAM.
...
backend dùng cho online FID, build thêm \texttt{celeba256\_source\_gmm.npz},
và giữ cadence checkpoint mặc định ở mốc \texttt{210000} step. Block
