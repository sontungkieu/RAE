...
\begin{itemize}[leftmargin=1.5em]
  ...
  \item mặc định tự materialize checkpoint \texttt{vae\_trial1.pkl} cục bộ từ
  \texttt{stabilityai/sd-vae-ft-mse}, trừ khi
  \texttt{stage\_1.params.pretrained\_path} đã trỏ tới pickle sẵn có;
  \item không bắt buộc phải bootstrap latent stat trước khi chạy flow CelebA public;
...
cần, đồng thời public notebook flow của CelebA tập trung vào
\texttt{StabilityVAE + SiT-B + moe1}. Objective trên đường JAX mặc định vẫn đi
qua interface \texttt{sit} cho cả hai dạng backbone, nhưng có thể chuyển sang
\texttt{sit\_gmm\_moe1} khi config có block \texttt{source}.
...
\begin{itemize}[leftmargin=1.5em]
  ...
  \item có thể thay pure Gaussian source bằng learned source
  \texttt{GMM + SourceMoE};
...
\midrule
\endhead
\texttt{src\_jax/vendor.py} & Bootstrap \texttt{diffuse\_nnx} vào \texttt{\~{}/.cache/rae\_jax/diffuse\_nnx}, pin commit, rồi overlay các file backend cho \texttt{moe1} với manifest hash và lock file \\
\texttt{src\_jax/config\_adapter.py} & Chuyển YAML OmegaConf của repo sang config backend NNX, map \texttt{SiT}/\texttt{SiTDH}/\texttt{StabilityVAE} sang backend phù hợp, forward \texttt{random\_flip} / prefetch knobs, và chuyển từ \texttt{sit} sang \texttt{sit\_gmm\_moe1} khi bật \texttt{source.enabled} \\
\texttt{src\_jax/stage2\_runtime.py} & Train, sampling, load checkpoint PyTorch hoặc Orbax, JAX validation-loss integration, FID glue, wandb bridge, và source-prior-aware preview / sample / FID \\
 ...
 \texttt{src\_jax/build\_source\_gmm.py} & Encode dữ liệu qua \texttt{stage1.StabilityVAE}, flatten latent đã scaled, fit diagonal GMM, và ghi artifact cho block \texttt{source} \\
 \texttt{src\_jax/export\_celebahq\_hf.py} & Download \texttt{eurecom-ds/celeba-hq-256} từ Hugging Face rồi export thành cây \texttt{ImageFolder} cho các thử nghiệm CelebA-HQ thủ công \\
 \texttt{src\_jax/export\_celebahq\_tfds.py} & Export TFDS \texttt{celeb\_a\_hq/256} thành cây \texttt{ImageFolder} cho các thử nghiệm CelebA-HQ thủ công và ép protobuf runtime kiểu Python để tránh lỗi import TFDS trên Kaggle \\
...
\texttt{src\_jax/moe1/} & Chứa GMM utils, \texttt{SourceMoE}, và source-side losses dùng chung giữa repo và backend overlay \\
...
Với branch \texttt{jax-vae-sit-moe1}, checkpoint Stage 2 phải khớp shape
