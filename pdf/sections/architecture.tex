...
\begin{itemize}[leftmargin=1.5em]
  ...
  \item không bắt buộc phải bootstrap latent stat trước khi chạy CelebA;
...
\texttt{src\_jax/config\_adapter.py} & Chuyển YAML OmegaConf của repo sang config backend NNX, map \texttt{SiT}/\texttt{SiTDH}/\texttt{StabilityVAE} sang backend phù hợp, forward \texttt{random\_flip} / prefetch knobs, và giữ interface train \texttt{sit} cho cả flow DH lẫn single-tower VAE \\
...
Với branch \texttt{jax-vae-sit}, checkpoint Stage 2 phải khớp shape
