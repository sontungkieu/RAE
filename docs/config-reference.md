...
- `src_jax/build_source_gmm.py` now stores the feature extractor used for GMM
  fitting inside the artifact itself. On DH/RAE branches the default `auto`
  mode still switches to `pyramid_16k`, which pools `16 x 16 x 768` NHWC
  latents down to a deterministic `4 x 4 x 1024` feature map before flattening
  to `16384` dimensions for the GMM. On high-RAM hosts you can force
  `flatten`; the builder now preallocates one `N x D` float32 matrix and
  standardizes it in place, so the full flattened latent vector no longer
  needs extra disk-backed staging just to stay under RAM limits. Use
  `spatial_mean` only as the low-RAM fallback when you cannot afford the
  larger offline feature matrix
