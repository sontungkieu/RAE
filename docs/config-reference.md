...
- `src_jax/build_source_gmm.py` now stores the feature extractor used for GMM
  ...
  mode now switches to `pyramid_16k`, which pools `16 x 16 x 768` NHWC latents
  down to a deterministic `4 x 4 x 1024` feature map before flattening to
  `16384` dimensions for the GMM. Use `spatial_mean` only as the low-RAM
  fallback when you cannot afford the larger offline feature matrix
