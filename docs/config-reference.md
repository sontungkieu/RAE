...
- `hidden_channels`: CNN trunk width for the source MoE; the CelebA-HQ RAE
  notebooks widen this to `256` because the latent tensor is `16 x 16 x 768`
...
- `balance_loss_weight`, `entropy_loss_weight`, `var_kl_loss_weight`: auxiliary
  source losses added on top of the flow-matching objective
