...
- `src_jax/build_source_gmm.py` now stores the feature extractor used for GMM
  fitting inside the artifact itself. On DH/RAE branches the repo default is
  `flatten`, so the GMM sees the full `16 x 16 x 768 -> 196608` latent
  vector unless you explicitly switch to a condensed extractor such as
  `pyramid_16k` or `spatial_mean`
- the builder also now separates the backing dtype from the EM compute dtype:
  `--storage-dtype auto` resolves to `float16` for large flattened DH runs,
  which cuts the offline feature matrix footprint roughly in half, while
  `--compute-dtype auto` keeps the GMM fit itself in `float32` chunk by chunk
- `pyramid_16k` still pools `16 x 16 x 768` NHWC latents down to a
  deterministic `4 x 4 x 1024` feature map before flattening to `16384`
  dimensions for the GMM; use `spatial_mean` only as the low-RAM fallback when
  you cannot afford the larger offline feature matrix
