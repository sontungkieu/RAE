...
- [raes-jax-celeba-kaggle-tpuv5e8-sitdh-b-moe1.ipynb](raes-jax-celeba-kaggle-tpuv5e8-sitdh-b-moe1.ipynb): sibling Kaggle `TPU v5e-8` notebook for the DH/two-tower `SiTDH-B + moe1` recipe, using `hidden_size=[768, 2048]`, `depth=[12, 2]`, `num_heads=[12, 16]`, `use_pos_embed=true`, and an explicit `pyramid_16k` GMM build step that writes `celeba256_source_gmm_pyr16k.npz` before training
...
under `generated_notebooks/sitdh_moe1_pyr16k/` switch to `WANDB_Tung`. They no
longer read `HF_TOK_WRITE_KAGGLE`
...
- `src_jax/build_source_gmm.py` still supports the full flattened RAE latent on the DH branches, including the newer `--storage-dtype auto` / `--compute-dtype auto` safeguards that keep very wide `flatten` runs off the old `float32`-only path and auto-promote offline EM back to `float32` if someone explicitly forces `--compute-dtype float16`. The shipped DH Kaggle notebooks on this branch now call the builder explicitly with `--feature-extractor pyramid_16k` and write `*_source_gmm_pyr16k.npz` artifacts so the default branch workflow no longer relies on full flatten. `--feature-extractor spatial_mean` remains the low-RAM fallback when even `pyramid_16k` is too large for the host.
...
- `raes-jax-celeba-kaggle-tpuv5e8-sitdh-b-moe1.ipynb` is the `TPU v5e-8` sibling notebook for the DH/two-tower `SiTDH-B + moe1` recipe, reusing the same Kaggle/JAX flow while writing `CelebA256_SiTDH-B_DINOv2-B_moe1_jax_tpuv5e8.yaml`, setting `hidden_size=[768, 2048]`, `depth=[12, 2]`, and `num_heads=[12, 16]`, and building the source artifact explicitly as `celeba256_source_gmm_pyr16k.npz` via `--feature-extractor pyramid_16k` before training. This notebook also keeps `src_jax/build_fid_stats.py --num-workers 32` by default.
- `raes-jax-celeba-kaggle-tpuv5e8-sitdh-b-moe1-resume.ipynb` is the resume-only notebook for the `SiTDH-B + moe1` variant: it still restores the archived workdir, checks that `celeba256_source_gmm_pyr16k.npz` is present, clones the newest `checkpoint_*` from the latest `CelebA256_SiTDH-B_DINOv2-B_moe1_jax_tpuv5e8-*` workdir into a fresh timestamped resume workdir, seeds a fresh `wandb_run.json`, and resumes from there while keeping the original experiment name.
...
- For numbered DH ablations on Kaggle, run `python3 scripts/generate_ablation_notebooks.py --spec configs/ablation/celeba_sitdh_moe1_pyr16k.yaml --overwrite`. The generator materializes `generated_notebooks/sitdh_moe1_pyr16k/*.ipynb`, pins every run to `pyramid_16k`, switches generated notebooks to the Kaggle secret `WANDB_Tung`, and injects `--wandb-group/--wandb-tags` metadata so the resulting runs land under the shared W&B group `celeba-sitdh-moe1-pyr16k-ablation`.
