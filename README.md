...
- `src_jax/build_source_gmm.py` encodes images with the frozen RAE and still auto-selects a structured `pyramid_16k` GMM feature extractor for very large NHWC latents. On the DH branches that means pooling the `16 x 16 x 768` latent grid down to a deterministic `4 x 4 x 1024` representation before fitting the GMM, so the learned source keeps far more spatial information than the temporary `spatial_mean` fallback. On high-RAM hosts you can now also force `--feature-extractor flatten`: the builder preallocates one `N x D` float32 matrix and the GMM standardization now runs in place, so the full `196608`-dimensional latent vector no longer requires disk-backed memmaps just to avoid extra host copies.
