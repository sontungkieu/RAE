...
- `src_jax/train.py` now also accepts `--wandb-group` and comma-separated `--wandb-tags`, and forwards both directly into `wandb.init(...)` on the JAX path so generated ablation notebooks can keep short run names while still grouping runs cleanly on the dashboard.
...
- `python3 scripts/generate_ablation_notebooks.py --spec configs/ablation/celeba_vae_moe1.yaml --overwrite` now emits numbered Kaggle TPU train notebooks under `generated_notebooks/vae_moe1/`, using short names such as `01-moe1-m4-tau2-vk1.ipynb` while still wiring a unique GMM artifact path, a generated Stage 2 config path, a dedicated results root, and W&B `group/tags` for each ablation run.
