...
- [raes-jax-celeba-kaggle-tpuv5e8-sitdh-b-moe1-resume.ipynb](raes-jax-celeba-kaggle-tpuv5e8-sitdh-b-moe1-resume.ipynb): minimal `TPU v5e-8` resume-only notebook for the `SiTDH-B + moe1` variant, auto-detecting the newest `CelebA256_SiTDH-B_DINOv2-B_moe1_jax_tpuv5e8-*` Orbax run, requiring the persisted `celeba256_source_gmm.npz` artifact, then cloning the newest checkpoint into a fresh timestamped resume workdir with a fresh `wandb_run.json` so the resumed job keeps the original experiment name but logs to a new W&B run
...
- `src_jax/build_source_gmm.py` now defaults to the full flattened RAE latent when fitting the diagonal GMM. On the DH branches that means keeping the full `16 x 16 x 768 -> 196608` feature vector while still using the low-copy builder path: the script preallocates one `N x D` backing matrix, but large `flatten` runs now resolve `--storage-dtype auto` to `float16` so host RAM stays closer to half the old footprint. The offline EM path then upcasts chunks back to `float32` by default via `--compute-dtype auto`; if someone explicitly requests `float16` compute for a very wide flattened DH run, the fitter now promotes the offline KMeans/EM math back to `float32` automatically to avoid overflow and NaN stalls. `--feature-extractor pyramid_16k` remains available when you explicitly want a structured compressed feature, while `spatial_mean` stays the low-RAM fallback.
...
- `raes-jax-celeba-kaggle-tpuv5e8-sitdh-b-moe1-resume.ipynb` is the resume-only Kaggle TPU notebook for the latest `CelebA256_SiTDH-B_DINOv2-B_moe1_jax_tpuv5e8-*` Orbax workdir, keeps the persisted `celeba256_source_gmm.npz` artifact in place, copies the newest `checkpoint_*` into a new timestamped resume workdir, writes a fresh `wandb_run.json` with a new run ID, and keeps the original experiment name for lineage.
