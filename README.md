...
- [raes-jax-celebahq-kaggle.ipynb](raes-jax-celebahq-kaggle.ipynb): Kaggle notebook for the matching CelebA-HQ 256 JAX flow, exporting the dataset into `celebahq256_imgfolder`
...
- the same branch now also keeps the parallel CelebA-HQ notebook set and exporter helpers: `src_jax/export_celebahq_hf.py`, `src_jax/export_celebahq_tfds.py`, plus `raes-jax-celebahq-kaggle*.ipynb` for the `SiTDH-S` and `SiTDH-B` 256 flows.
