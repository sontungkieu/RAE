...
- `src_jax/build_source_gmm.py` encodes images with the frozen RAE and now auto-selects a compact GMM feature extractor for very large NHWC latents. On the DH branches this means `spatial_mean` pooling over the `16 x 16` grid by default, which keeps CelebA/CelebA-HQ GMM fitting within Kaggle host memory instead of flattening all `196608` latent dimensions at once.
