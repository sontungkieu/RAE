...
* Public CelebA workflows centered on backend-native `StabilityVAE + SiT-B + moe1`.
...
- [vaes-jax-celeba-kaggle-tpuv5e8-sitb-moe1-resume.ipynb](vaes-jax-celeba-kaggle-tpuv5e8-sitb-moe1-resume.ipynb): resume-only `TPU v5e-8` notebook for the timestamped `CelebA256_SiT-B_StabilityVAE_moe1_jax_tpuv5e8-*` Orbax runs; it now clones the newest `checkpoint_*` into a fresh timestamped workdir, seeds a fresh `wandb_run.json`, and resumes with the original experiment name but a new W&B run ID
- [configs/ablation/celeba_vae_moe1.yaml](configs/ablation/celeba_vae_moe1.yaml): source-of-truth ablation spec for numbered CelebA `StabilityVAE + SiT-B + moe1` Kaggle TPU notebooks
- [generated_notebooks/vae_moe1/manifest.csv](generated_notebooks/vae_moe1/manifest.csv): generated ablation manifest mapping each numbered notebook to its GMM artifact, W&B metadata, and key source hyperparameters
- [vaes-jax-celebahq-kaggle-tpuv5e8-sitb-moe1.ipynb](vaes-jax-celebahq-kaggle-tpuv5e8-sitb-moe1.ipynb): `TPU v5e-8` notebook for the CelebA-HQ 256 `StabilityVAE + SiT-B + moe1` flow
- [vaes-jax-celebahq-kaggle-tpuv5e8-sitb-moe1-resume.ipynb](vaes-jax-celebahq-kaggle-tpuv5e8-sitb-moe1-resume.ipynb): resume-only `TPU v5e-8` notebook for the timestamped `CelebAHQ256_SiT-B_StabilityVAE_moe1_jax_tpuv5e8-*` Orbax runs, using the same fresh-workdir and fresh-W&B-run resume flow as the CelebA notebook

The standard Kaggle notebooks on this branch now only bootstrap the `WANDB2`
...
The generated ablation notebooks under `generated_notebooks/vae_moe1/` use the
separate `WANDB_Tung` secret so high-volume sweeps do not reuse the default
notebook credential name.
...
- `src_jax/build_source_gmm.py` builds the offline diagonal GMM artifact used by the learned `source` block on the JAX `moe1` path.
...
- the public `vaes-jax-celeba-*.ipynb` notebooks now prepare `jessicali9530/celeba-dataset` directly into `/kaggle/working/celeba256_imgfolder`, center-cropping and resizing images to `256x256` in the same `ImageFolder` layout used by the existing JAX runtime.
- `src_jax/export_celebahq_hf.py` and `src_jax/export_celebahq_tfds.py` remain available if you still want a manual CelebA-HQ export path; the TFDS helper still requires the manual tar files under `--manual-dir`, and it forces the pure-Python protobuf runtime before importing TFDS so Kaggle does not trip over the common descriptor-compatibility crash.
...
- `training.random_flip` and `eval.random_flip` now control the raw-image transform on the JAX Stage 2 path; the public CelebA configs on this branch default training-time horizontal flips on, while eval/FID stays unflipped unless you opt in explicitly.
...
- the shipped TPU notebooks on this branch now default `export PROJECT="moe-diffusion"`; the existing `run_name` strings already encode the dataset, `StabilityVAE`, backbone, `moe1`, and TPU pipeline, so no extra suffix was added to the experiment name.
...
- `src_jax/train.py` now also accepts `--wandb-group` and comma-separated `--wandb-tags`, and forwards both directly into `wandb.init(...)` on the JAX path so generated ablation notebooks can keep a shared study `group` while each run still has its own full slug plus timestamp.
...
- `vaes-jax-celeba-kaggle-tpuv5e8-sitb-moe1-resume.ipynb` now copies the latest `checkpoint_*` from `CelebA256_SiT-B_StabilityVAE_moe1_jax_tpuv5e8-*` into a fresh timestamped resume workdir, seeds a fresh `wandb_run.json` with a new W&B run ID, keeps the original experiment name for readability, and re-applies `training.log_rae_latent_stats=true` plus `training.log_activation_stats=true` from the CLI so resumed runs keep those diagnostics on by default.
- `python3 scripts/generate_ablation_notebooks.py --spec configs/ablation/celeba_vae_moe1.yaml --overwrite` now emits numbered Kaggle TPU train notebooks under `generated_notebooks/vae_moe1/`, using explicit names such as `01-moe1-m4-tau2-vk1-bl01-ent001-tv1-cd16-hc64.ipynb`. Each generated notebook rewrites its title to match the filename stem, keeps `wandb_group=celeba-vae-moe1-ablation`, emits detailed W\&B tags such as `study:*`, `dataset:*`, `model:*`, `stage1:*`, `source:*`, `modes:*`, `tau:*`, `var_kl:*`, `balance:*`, `entropy:*`, `target_var:*`, `cond_dim:*`, and `hidden:*`, and now pins `training.total_steps=290000` so CelebA ablation runs do not stop after the adapter's one-epoch fallback at roughly `20k` steps.
- those generated notebooks now also rewrite the config preview cell so it reads the run-specific Stage 2 YAML under `configs/ablation/generated/celeba_vae_moe1/` instead of the base template path.
...
- The same branch now also keeps the parallel CelebA-HQ TPU `moe1` notebook set: `vaes-jax-celebahq-kaggle-tpuv5e8-sitb-moe1.ipynb` and `vaes-jax-celebahq-kaggle-tpuv5e8-sitb-moe1-resume.ipynb`. They reuse the same `StabilityVAE + SiT-B + moe1` recipe while exporting `/kaggle/working/celebahq256_imgfolder`, copying the newest checkpoint from `CelebAHQ256_SiT-B_StabilityVAE_moe1_jax_tpuv5e8-*` into a fresh timestamped resume workdir, and pinning the same Stage 2 host-loader/diagnostic defaults (`training.num_workers=16`, `training.prefetch_factor=4`, `eval.prefetch_factor=4`, `training.log_rae_latent_stats=true`, `training.log_activation_stats=true`).
