...
- [raes-jax-celeba-kaggle-tpuv5e8-sitdh-s-moe1.ipynb](raes-jax-celeba-kaggle-tpuv5e8-sitdh-s-moe1.ipynb): Kaggle `TPU v5e-8` notebook for the `SiTDH-S + moe1` CelebA variant, keeping host-side CPU FID, applying the automatic `jaxlib` executable-stack fix, and generating the learned-source Stage 2 config `CelebA256_SiTDH-S_DINOv2-B_moe1_jax_tpuv5e8.yaml`
...
- `src_jax/build_source_gmm.py` encodes images with the frozen RAE and now auto-selects a structured `pyramid_16k` GMM feature extractor for very large NHWC latents. On the DH branches that means pooling the `16 x 16 x 768` latent grid down to a deterministic `4 x 4 x 1024` representation before fitting the GMM, so the learned source keeps far more spatial information than the temporary `spatial_mean` fallback while still avoiding the full `196608`-dimensional flattening path. If host RAM is tight, you can still force `--feature-extractor spatial_mean` explicitly.
...
- `raes-jax-celeba-kaggle-tpuv5e8-sitdh-s-moe1.ipynb` copies that flow for `TPU v5e-8`, fixes Stage 2 to `SiTDH-S`, clears the `jaxlib` executable-stack flag that Kaggle can reject, builds the JAX `fid_ref`, builds `celeba256_source_gmm.npz`, and trains with the learned source prior instead of pure Gaussian initialization. On this DH branch the notebook keeps `src_jax/build_fid_stats.py --num-workers 32` as the default host setting.
...
- `raes-jax-celeba-kaggle-tpuv5e8-sitdh-b-moe1-resume.ipynb` is the resume-only notebook for the `SiTDH-B + moe1` variant: it still restores the archived workdir, but now also checks that `celeba256_source_gmm.npz` is present and resumes the newest `CelebA256_SiTDH-B_DINOv2-B_moe1_jax_tpuv5e8-*` run.
