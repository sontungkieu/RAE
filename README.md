...
- `src_jax/build_source_gmm.py` now defaults to the full flattened RAE latent when fitting the diagonal GMM. On the DH branches that means keeping the full `16 x 16 x 768 -> 196608` feature vector while still using the low-copy builder path: the script preallocates one `N x D` float32 matrix and the GMM standardization runs in place, so the full latent vector no longer requires disk-backed memmaps just to avoid extra host copies. `--feature-extractor pyramid_16k` remains available when you explicitly want a structured compressed feature, while `spatial_mean` stays the low-RAM fallback.
